...
public actor AgentLiveTeamRouter: LiveTeamCoordinatorRouting, LiveTeamOverseerRouting,
    ...
    static func overseerDeveloperInstructions(policy: String) -> String {
        ...
        The CEO position is already occupied and cannot be assigned as a worker position. Choose each position because its owned contribution and allowed capabilities match real work in the funded bet. Developer has no default priority. Every assignment must name the required contribution and acceptance evidence, remain inside that position's capabilities, and end with a stopping or capability-block condition. When evidence exposes a capability block, hire or assign the profession and target that can perform it; never instruct the blocked worker to cross the boundary. Name each assignment with a two-to-five-word, outcome-led headline using concrete terms from the goal. Use one-time people for one-time work and recurring people for sustained work. Select every target from the offered target IDs. Prefer Own memory. Use Fresh every run for independent validation. Use Shared memory only when the professions have the same tool policy and benefit from the same conversation. You alone decide when the fixed goal is complete and the correct company has no people needed beyond you. Return exactly the requested JSON object.
    ...
    private static func modeInstruction(_ mode: LiveTeamOverseerMode) -> String {
        ...
        case .strategicReview:
            "Make an investment decision. Return keep only when the same funded bet and professions deserve another funding window unchanged. Return revise with one complete replacement company and product bet when a new bet, assignment, target, hire, or position would materially increase return. A capability block requires you to assign a target and profession that can perform the effect, narrow the assignment, or stop the bet; never send the same work back across the blocked profession boundary. For a legacy setup without a persisted CEO, predefined positions, and funded product bet, revise is mandatory. Return complete only when profession-appropriate durable evidence satisfies the entire fixed user goal and the correct company has no remaining work. You cannot ask the user to manage internal stages. For keep or complete, set workingGoal, productBet, members, overseerTargetID, and preferredNextMemberID to null."
        case .completionReview:
